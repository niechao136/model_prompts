...
function main({body}) {
  const obj = JSON.parse(body)
  const outputs = obj?.data?.outputs ?? {}
  const is_remote = Number(outputs?.is_remote) ?? 0
  return {
    result: outputs?.result ?? '',
    content: outputs?.content ?? '',
    type: outputs?.type ?? '',
    need_answer: is_remote === 1 ? 0 : 1,
  }
}
...
function main({body}) {
  ...
  const outputs = obj?.data?.outputs ?? {}
  const is_remote = Number(outputs?.is_remote) ?? 0
  const is_reboot = Number(outputs?.is_reboot) ?? 0
  const is_find_device = Number(outputs?.is_find_device) ?? 0
  return {
    result: outputs?.result ?? '',
    content: outputs?.content ?? '',
    type: outputs?.type ?? '',
    need_answer: is_remote === 1 || is_reboot === 1 || is_find_device === 1 ? 0 : 1,
  }
}
